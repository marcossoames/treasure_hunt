...
int pfd[2];             // Pipe pentru comunicarea cu monitorul
...
void sig_list_treasures_handler(int signum)
{
    ...
    close(fd);
    unlink("hunt_id.tmp"); // Sterg fisierul temporar
    ...
    while ((entry = readdir(dir)) != NULL)
    {
        if ((strncmp(entry->d_name, "comoara", 7) == 0) && (strstr(entry->d_name, ".bin"))) // Verific ca fisierul sa fie unul de tip: comoaraX.bin
        {
            ...
            {
                char mesaj_eroare_fisier[256];
                snprintf(mesaj_eroare_fisier, sizeof(mesaj_eroare_fisier), "[MONITOR] Nu pot deschide fisierul %s!\n", path);
                write(1, mesaj_eroare_fisier, strlen(mesaj_eroare_fisier));
                continue;
            }
...
void sig_view_treasure_handler(int signum)
{
    ...
    close(fd);
    unlink("view_treasure.tmp"); // Sterg fisierul temporar
...
void start_monitor()
{
    ...
    if (pipe(pfd) < 0) // Creez pipe-ul pentru comunicarea dintre procese
    {
    ...
    if (pid == 0)
    {
        ...
        // Redirectionez stdout catre pipe
        dup2(pfd[1], STDOUT_FILENO);
        close(pfd[1]); // Inchid pfd[1] dupa dup2
...
int main(void)
{
    ...
    while (1) // Bucla infinita pentru a astepta comenzi
    {
        ...
        char *comanda = "\nDati comanda (start_monitor / list_hunts / list_treasures <hunt_id> / view_treasure <hunt_id> <id> / calculeaza_scor / stop_monitor / exit):\n\n";
        ...
        */
        else if (strcmp(buffer, "list_hunts") == 0)
        {
            ...
            {
                char *mesaj_listare_hunturi = "\nCerere de listare hunt-uri trimisa monitorului.\n";
                write(1, mesaj_listare_hunturi, strlen(mesaj_listare_hunturi)); // Afisez mesajul de cerere
                kill(monitor_pid, SIGUSR2);                                     // Trimite semnalul SIGUSR2 pentru a lista hunt-urile

                usleep(300000); // Pauza pentru procesare

                char buf[1024];                                  // Buffer pentru citirea datelor din pipe
                ssize_t nbytes = read(pfd[0], buf, sizeof(buf)); // Citesc datele din pipe
                if (nbytes > 0)                                  // Daca am citit date
                {
        ...
        else
        {
            ...
            else if (strcmp(buffer, "calculeaza_scor") == 0)
            {
                DIR *dir = opendir("."); // Deschid directorul curent
                if (dir == NULL)
                {
                    char *eroare_deschidere_director = "\nEroare la deschiderea directorului curent.\n";
                    write(1, eroare_deschidere_director, strlen(eroare_deschidere_director));
                    continue;
                }

                struct dirent *entry;                  // Structura pentru a citi un director
                while ((entry = readdir(dir)) != NULL) // Parcurg toate fisierele din directorul curent
                {
                    if (strncmp(entry->d_name, "hunt", 4) == 0) // Verific ca fisierul sa fie unul de tip: huntX
                    {
                        //printf("%s\n", entry->d_name); // Afisez numele directorului
                        int pf[2];        // Pipe pentru comunicarea dintre procese
                        if (pipe(pf) < 0) // Creez pipe-ul pentru comunicarea dintre procese
                        {
                        ...
                        pid_t pid = fork(); // Creez un nou proces (copil)
                        if (pid < 0)
                        {
                            char *eroare_fork = "\nEroare la fork.\n";
                            write(1, eroare_fork, strlen(eroare_fork));
                            continue;
                        }
                        ...
                        {
                            // Procesn copil
                            close(pf[0]);                                             // Inchid capatul de scriere
                            dup2(pf[1], STDOUT_FILENO);                               // Redirectionez stdout catre pipe
                            close(pf[1]);                                             // Inchid pfd[1] dupa dup2
                            execl("./cs", "calculate_score", entry->d_name, NULL); // Execut procesul score_calc
                            exit(1);                                                  // Daca execl esueaza, ies din procesul copil
                        }
                        else
                        {
                            // Proces parinte
                            close(pf[1]);  // Inchid capatul de scriere
                            char buf[512]; // Buffer pentru citirea datelor din pipe
                            ssize_t numar_octeti_cititi = 0;

                            // Continui sa citesc pana cand read() returnează 0 (EOF) sau <0 (eroare)
                            while (1)
                            {
                                // Citesc in buffer de la capatul de citire al pipe-ului
                                numar_octeti_cititi = read(pf[0], buf, sizeof(buf));

                                // Daca am terminat de citit (EOF), ies din bucla
                                if (numar_octeti_cititi == 0)
                                {
                                    break;
                                }
                                ...
                                if (numar_octeti_cititi < 0)
                                {
                                    char *eroare_citire_pipe = "\nEroare la citirea din pipe.\n";
                                    write(1, eroare_citire_pipe, strlen(eroare_citire_pipe));
                                    break;
                                }

                                // Altfel, afisez pe ecran ce am primit
                                write(1, buf, numar_octeti_cititi);
                            }

                            close(pf[0]); // Inchid capatul de citire
                            //wait(NULL);   // Astept ca procesul copil sa se termine
                            waitpid(pid, NULL, 0); // Asteapta doar procesul score_calc

                        }
                    }
                }

                closedir(dir); // Inchid directorul curent
                char *mesaj_finalizare = "\nCalcularea scorului s-a terminat.\n";
                write(1, mesaj_finalizare, strlen(mesaj_finalizare));
            }
